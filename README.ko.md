...
- 카테고리 필터, 키워드 검색 — 상황 키워드·영문 동의어 지원("등장", "loading", "slide" 모두 검색 가능)
- 모든 카드에 렌더링 비용(Composite/Paint/Layout) 배지 표시, 호버 시 설명 제공
- 카드 단위 다시 재생 / 전체 다시 재생 — 모션을 일시정지한 상태에서 다시 재생을 누르면 그 카드만 재생됩니다(나머지는 정지 유지)
