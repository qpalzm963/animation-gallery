...
- カテゴリフィルタ、キーワード検索——シーン語・英語シノニムに対応（「入場」「loading」「slide」でもヒット）
- 各カードにレンダリングコスト（Composite／Paint／Layout）バッジを表示、ホバーで解説
- 単一カードのリプレイ／すべて再生——動きを一時停止した状態でリプレイを押すと、そのカードだけを再生できます（他は静止のまま）
