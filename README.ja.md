...
**76 種類のウェブアニメーション技法**を収録した、単一ページのインタラクティブ図鑑です。
ありふれたフェードや移動から、FLIP、View Transitions、スクロールタイムライン、グーイーエフェクト、グリッチアートといった少し珍しい手法まで。どのカードも**実際に動いている**実装で、カードにホバーするか「リプレイ」を押すともう一度再生でき、使いどころの解説と、そのままコピーできるコアコードが付いています。
...
- **クロスプラットフォーム対照表** — CSS / Web、Flutter、SwiftUI の概念対照表と、各プラットフォーム固有の表現
  （SwiftUI の `KeyframeAnimator`、`PhaseAnimator`、Liquid Glass；Flutter の `Hero`、`Curves`）
- **デュレーションとイージングの早見表**
